...
A daylight-aware calendar MVP that mimics Google Calendar's week/month views while shading each day based on local sunrise and sunset. It pulls read-only events from your Google Calendar using the browser-only Google Identity Services flow—no server required.

## Features

- Google sign-in via OAuth (read-only scope) with calendar Picker toggle.
...
- **Rendering**:
  ...
  - `EventPopover` shows quick details with meeting links when an event is selected.
